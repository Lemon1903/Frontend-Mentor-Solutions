...
export default function Countries() {
	...
	return (
		<div className="space-y-16">
			...
			>
				<AnimatePresence>
					{isLoading && generateSequence(itemsPerPage).map(
						(num) => <Card.Loading key={num} />
					)}
			...
			<div className="mx-4 flex flex-wrap justify-center gap-6 md:mx-8 xl:mx-0">
				{pagesLength > 1 && generateSequence(pagesLength).map((num) => (
					<PrimaryButton
						className={`${getActiveStyle(num)} aspect-square w-10 hover:scale-110 sm:w-12 md:w-16`}
						key={num}
